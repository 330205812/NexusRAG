# Modify from langchain RecursiveCharacterTextSplitter
...
class TextSplitter(BaseDocumentTransformer, ABC):
    ...
    def create_documents(
        ...
        """Create documents from a list of texts and return a tuple of:
        (list of Document objects, list of content strings, list of hash values).
        """
        ...
        for i, text in enumerate(texts):
            ...
            for chunk in self.split_text(text):
                ...
                if contains_text(page_content):
                    hash_value = hashlib.sha256(page_content.encode('utf-8')).hexdigest()[:16]
...
if __name__ == "__main__":
    ...
    # file_path = "/home/zhangwq/project/szjw/work_dir/processed/kb_5ecc56689a52ca85_4dd538c0fe423325_20250224_162905/在纪念乔石同志诞辰100周年座谈会上的讲话/在纪念乔石同志诞辰100周年座谈会上的讲话.txt"
    # file_path = "/home/zhangwq/project/szjw/work_dir/processed/kb_5ecc56689a52ca85_4dd538c0fe423325_20250224_162905/测试wps的文档/95fc4229bac4db21_20250224_162950.text"
    file_path = "/home/zhangwq/project/szjw/work_dir/processed/kb_5ecc56689a52ca85_4dd538c0fe423325_20250224_162905/中国共产党纪律处分条例/72dd486189e46574_20250224_162951.text"
    ...
    # texts = []
    # for item in data:
    #     conversation = f"id: {item['id']}\n" + "\n".join([f"{msg['from']}: {msg['value']}" for msg in item["conversations"]])
    #     texts.append(conversation)

    # 假设 self.splitter.create_documents(texts) 处理数据
    # splitter = MarkdownParser(max_chunk_size=chunk_size)
    # documents = splitter.create_documents(texts)

    start_time = time.time()
    splitter = RecursiveCharacterTextSplitter(chunk_size=1200, chunk_overlap=200)
    # pdb.set_trace()
    final_chunks, content_list, hash_list = splitter.create_documents([content])
    end_time = time.time()
    logger.info(f"time:{end_time - start_time}")

    # 方法2：使用循环逐个输出元素
    logger.info("final_chunks 内容:")
    for i, chunk in enumerate(final_chunks):
        logger.info(f"[{i}] {chunk}")
    logger.debug(f"final_chunks 长度: {len(final_chunks)}")
    logger.info(f"----" * 30)

    logger.info("content_list 内容:")
    for i, content in enumerate(content_list):
        logger.info(f"[{i}] {content}")
    logger.debug(f"content_list 长度: {len(content_list)}")
    logger.info(f"----" * 30)

    logger.info(f"hash_list: {hash_list}")
    logger.debug(f"hash_list 长度: {len(hash_list)}")
